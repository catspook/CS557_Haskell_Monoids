...
> train = mconcat . map (\(zs) -> case zs of
>                                   [] -> singletonFreqMap "" ""
>                                   [x] -> singletonFreqMap x "" 
>                                   (x:y:zs) -> singletonFreqMap x y) . map words . lines
> --train = mconcat . map bigramFreqMap . map words . lines

call l in lines, do words (l) -> w
for w, call bigrams
then call the middle of bigramFreqMap 
